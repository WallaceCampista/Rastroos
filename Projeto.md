...
- [x] **Ajustes pedidos depois**: espaço entre o cabeçalho e as ações; pagar/reabrir fatura, marcar pago e excluir lançamento agora vão por `fetch` — os cards, os chips de mês e o streak por trás são atualizados e o detalhe é redesenhado no lugar, sem recarregar a página (o modal não fecha e reabre). Sem o aviso duplicado dentro do modal: o toast passou para cima do fundo desfocado (z-index 110) e os emojis de comemoração sobem entre o desfoque e a caixa do modal. A mensagem do servidor (`.screen > .flash`) fica escondida desde o primeiro pixel e só aparece como toast — acabou a piscada no topo das telas (usuários inclusive)
